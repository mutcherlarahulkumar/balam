...
func (r *SBRepo) List(year, month string, unpaidOnly bool) ([]models.SB, error) {
	query := `SELECT _id, policy_no, sb_duedate, sb_amount, i_no, sb_paydate, COALESCE(ch_no,'') AS ch_no, COALESCE(details,'') AS details FROM sb WHERE 1=1`
	args := []interface{}{}
	n := 1
...
func (r *SBRepo) ListByPolicy(policyNo int) ([]models.SB, error) {
	rows, err := r.db.Query(`SELECT _id, policy_no, sb_duedate, sb_amount, i_no, sb_paydate, COALESCE(ch_no,'') AS ch_no, COALESCE(details,'') AS details FROM sb WHERE policy_no=$1 ORDER BY sb_duedate`, policyNo)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *SBRepo) FindByID(id int) (*models.SB, error) {
	var item models.SB
	if err := r.db.QueryRow(`SELECT _id, policy_no, sb_duedate, sb_amount, i_no, sb_paydate, COALESCE(ch_no,'') AS ch_no, COALESCE(details,'') AS details FROM sb WHERE _id=$1`, id).
